...
interface AuthenticateUseCase {
    operator fun invoke(activity: Activity, contractType: String): Single<AuthorizeResponse>
}

class AuthenticateUseCaseImpl(private val repository: MainRepository): AuthenticateUseCase {

    override fun invoke(activity: Activity, contractType: String): Single<AuthorizeResponse> =
