...
fun DMEPushClient.authorizeOngoingPostbox(activity: Activity, credentials: DMEOAuthToken? = null): Single<Pair<DMEPostbox, DMEOAuthToken>> =
    Single.create<Pair<DMEPostbox, DMEOAuthToken>> { emitter ->
        createOngoingPostbox(activity, credentials) { postbox, credentials, error ->
            error?.let { emitter.onError(it) } ?: if (postbox != null && credentials != null)
